...
def crawl_real_cinnamon_cheese():
    """爬取真正的肉桂乳酪用户信息"""
    print("🧀 爬取真正的肉桂乳酪用户信息...")
    
    # 真正的肉桂乳酪用户信息
    target_user_name = "肉桂乳酪"
    target_user_id = "6538e8aa00000000060060e0"
    
    print(f"📋 目标用户: {target_user_name}")
    print(f"🔗 用户ID: {target_user_id}")
    print(f"🌐 真实URL: https://www.xiaohongshu.com/user/profile/{target_user_id}")
    
    # 获取或创建测试用户
    user, created = User.objects.get_or_create(
        username="tester_real_cinnamon_cheese",
        defaults={
            'email': 'tester_real_cinnamon_cheese@example.com',
            'first_name': '肉桂',
            'last_name': '乳酪'
        }
    )
    ...
    result = {
        ...
        'real_url': f'https://www.xiaohongshu.com/user/profile/{target_user_id}',
        'updates_count': len(updates),
        'updates': updates,
        'timestamp': datetime.now().isoformat(),
        'crawler_status': 'success' if updates else 'no_updates',
        'data_source': 'real_user_profile'
    }
    
    filename = f"real_cinnamon_cheese_crawler_result.json"
