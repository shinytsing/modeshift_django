...
"""
测试用例生成器完整流程测试
测试从用例产生大模型请求，到产生任务，到任务结束产生通知，到最后在节目可以点击下载用例
"""
...
def test_complete_flow():
        ...
        try:
            ...
            if status == "completed":
                ...
                if result:
                    ...
                    # 保存结果到文件
                    with open(f"test_result_{task_id[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", "w", encoding="utf-8") as f:
                        f.write(result)
                    print("💾 结果已保存到文件")
                break
            elif status == "failed":
    ...
    try:
        ...
        if response.headers.get('content-type') == 'text/plain; charset=utf-8':
            print("✅ TXT格式下载成功")
            # 保存下载的文件
            filename = f"download_txt_{task_id[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    try:
        ...
        if 'application/vnd.xmind.workbook' in response.headers.get('content-type', ''):
            print("✅ XMind格式下载成功")
            # 保存下载的文件
            filename = f"download_xmind_{task_id[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xmind"
            with open(filename, "wb") as f:
                f.write(response.content)
    ...
    try:
        ...
        if 'text/markdown' in response.headers.get('content-type', ''):
            print("✅ 飞书格式下载成功")
            # 保存下载的文件
            filename = f"download_feishu_{task_id[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
            with open(filename, "wb") as f:
                f.write(response.content)
