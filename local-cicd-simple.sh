...
# Black格式化检查
echo "检查代码格式..."
black --check --diff . || echo "代码格式检查失败"

# isort导入排序检查
echo "检查导入排序..."
isort --check-only --diff . || echo "导入排序检查失败"

# flake8代码风格检查
echo "检查代码风格..."
flake8 . || echo "代码风格检查失败"
...
# Bandit安全扫描
echo "运行Bandit安全扫描..."
bandit -r . -f json -o bandit-report.json || echo "Bandit扫描失败"
...
# 5. 数据库迁移测试
echo "🗄️ 数据库迁移测试..."
python manage.py makemigrations --check || echo "有未应用的迁移"
python manage.py migrate --check || echo "迁移检查失败"

# 6. 单元测试
echo "🧪 运行单元测试..."
python -m pytest tests/ -v --tb=short --cov=. --cov-report=html --cov-report=xml || echo "单元测试失败"
