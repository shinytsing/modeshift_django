...
def test_ningbo_ruanxiaoer_real():
    """测试宁波阮小二的真实小红书数据"""
    print("🔍 开始测试宁波阮小二的真实小红书数据...")
    
    # 使用从图片中获取的正确用户ID
    target_user_id = "758732836"  # 宁波阮小二的小红书号
    ...
    user, created = User.objects.get_or_create(
        username="tester_ningbo_ruanxiaoer_real",
        defaults={
            'email': 'tester_ningbo_ruanxiaoer_real@example.com',
            'first_name': '宁波',
            'last_name': '阮小二'
        }
    )
    ...
    filename = f"real_crawler_result_ningbo_ruanxiaoer_{target_user_id}.json"
