...
class TencentGradingService:
    """腾讯混元批改服务类"""

    def __init__(self):
        """初始化腾讯混元批改服务"""
        from apps.tools.services.llm_service import TencentService

        self.tencent_service = TencentService()
        self.use_tencent = self.tencent_service.is_available()

        if self.use_tencent:
            logger.info("✅ 使用腾讯混元批改作业")
        else:
            logger.warning("⚠️  腾讯混元API未配置或不可用")

    def is_available(self) -> bool:
        """检查服务是否可用"""
        return self.use_tencent
    ...
    def grade_homework_from_image(self, image_path: str) -> Dict[str, Any]:
        ...
        if not self.is_available():
            raise ValueError("腾讯混元服务不可用，请配置TENCENT_SECRET_KEY")

        try:
            logger.info(f"开始使用腾讯混元批改作业: {image_path}")
            ...
            # 使用TencentService（它支持图片输入）
            # 注意：TencentService可能需要特殊处理图片，这里先使用文本描述
            # 如果TencentService支持图片，可以直接传递
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"{prompt}\n\n图片已编码为base64，请根据图片内容进行批改。"
                }
            ]

            # 调用腾讯混元API
            response_text = self.tencent_service.generate_content(
                prompt=prompt,
                system_prompt="你是一个作业批改助手。你必须严格按照JSON格式返回结果，不要添加任何解释性文字。"
            )

            logger.info("腾讯混元批改完成")
            ...
            logger.error(f"腾讯混元批改失败: {str(e)}", exc_info=True)
            raise
...
1. **完整识别**：逐题识别，不要遗漏任何题目（包括选择题、填空题、计算题、主观题）
   - **重要**：如果OCR只识别到部分题目（如27-30），你必须根据图片内容识别所有题目（如25-30）
   - 仔细查看图片的每个区域，确保识别到所有题目
...
**现在请开始批改这份作业，确保识别所有题目并准确判题。**"""
    ...
    def _parse_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ 成功解析腾讯混元响应: {len(result['questions'])}道题目")
