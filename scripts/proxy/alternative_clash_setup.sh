...
echo "方法1: 尝试从GitHub releases下载..."
if wget -O clash.gz "https://github.com/Dreamacro/clash/releases/download/v1.16.0/clash-linux-amd64-v1.16.0.gz" 2>/dev/null; then
    echo "下载成功！"
    gunzip clash.gz
    chmod +x clash
elif wget -O clash.gz "https://github.com/Dreamacro/clash/releases/download/v1.15.1/clash-linux-amd64-v1.15.1.gz" 2>/dev/null; then
    echo "下载成功！"
    gunzip clash.gz
    chmod +x clash
elif wget -O clash.gz "https://github.com/Dreamacro/clash/releases/download/v1.14.0/clash-linux-amd64-v1.14.0.gz" 2>/dev/null; then
    echo "下载成功！"
    gunzip clash.gz
    chmod +x clash
else
    echo "GitHub下载失败，尝试其他方法..."
    
    # 方法2: 使用Go编译
    echo "方法2: 使用Go编译..."
    apt-get install -y golang-go git
    git clone https://github.com/Dreamacro/clash.git /tmp/clash-source
    cd /tmp/clash-source
    go build -ldflags "-X github.com/Dreamacro/clash/constant.Version=$(git describe --tags --abbrev=0)" -o /opt/clash/clash
    cd /opt/clash
    chmod +x clash
fi
...
if [ -f "/opt/clash/clash" ]; then
    echo "Clash安装成功！"
    /opt/clash/clash --version
else
    echo "Clash安装失败，尝试使用预编译版本..."
    # 方法3: 使用预编译的二进制文件
    wget -O clash "https://cdn.jsdelivr.net/gh/Dreamacro/clash@master/clash-linux-amd64" || \
    wget -O clash "https://raw.githubusercontent.com/Dreamacro/clash/master/clash-linux-amd64" || \
    curl -L -o clash "https://github.com/Dreamacro/clash/releases/download/premium/clash-linux-amd64-2023.08.17.gz" && gunzip clash
    chmod +x clash
fi
