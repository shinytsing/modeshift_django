...
if wget -O clash.gz "https://github.com/Dreamacro/clash/releases/download/v1.16.0/clash-linux-amd64-v1.16.0.gz" 2>/dev/null; then
    ...
    echo "GitHub下载失败，尝试其他源..."
    # 尝试从其他源下载
    if wget -O clash "https://cdn.jsdelivr.net/gh/Dreamacro/clash@master/clash-linux-amd64" 2>/dev/null; then
        echo "从CDN下载成功！"
    else
        echo "所有下载源都失败，请手动下载Clash二进制文件"
        echo "请访问: https://github.com/Dreamacro/clash/releases"
        echo "下载 clash-linux-amd64-*.gz 文件到 $TEMP_DIR 目录"
        echo "然后重新运行此脚本"
        exit 1
    fi
fi
...
echo "=== 上传Clash到服务器 ==="
# 上传Clash二进制文件到服务器
sshpass -p "$SERVER_PASSWORD" scp -o StrictHostKeyChecking=no "$TEMP_DIR/clash" "$SERVER_USER@$SERVER_IP:/tmp/"
