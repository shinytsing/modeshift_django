...
async def test_tools_pages_load():
    """测试工具页面负载"""
    print("\n🛠️ 测试工具页面负载")
    print("-" * 40)
...
async def test_burst_traffic_stress():
    """测试突发流量压力"""
    print("\n💥 测试突发流量压力")
    print("-" * 40)
...
async def main():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://localhost:8001/", timeout=aiohttp.ClientTimeout(total=5)) as response:
